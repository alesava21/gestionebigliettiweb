...
public class PrepareUpdateBigliettoServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String parametroIdDelBigliettoDaAggiornare = request.getParameter("idBiglietto");

		if (!NumberUtils.isCreatable(parametroIdDelBigliettoDaAggiornare)) {
		...
		try {
			request.setAttribute("bigliettoDaAggiornare", MyServiceFactory.getBigliettoServiceInstance()
					.caricaSingoloElemento(Long.parseLong(parametroIdDelBigliettoDaAggiornare)));
		} catch (Exception e) {
			// qui ci andrebbe un messaggio nei file di log costruito ad hoc se fosse attivo
			e.printStackTrace();
			request.setAttribute("errorMessage", "Attenzione si è verificato un errore.");
